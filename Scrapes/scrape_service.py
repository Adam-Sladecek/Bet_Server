...
import asyncio
import threading
from Models import Event, Odd, EventModel, OddModel
import logging 
    ...
    try:
        ...
        while True:
            ...
            if len(result_dictionary[result.request.sport_id]) == neccessaryCount:
                print("Scraping", result.request.sport_id)
                calc_thread = threading.Thread(target = scrape_sport, args = (result.request.sport_id, result.request.sport_name))
...
def scrape_sport(sport_id: int, sport_name: str): 
    ...
    print("End of scrape", sport_name)    
