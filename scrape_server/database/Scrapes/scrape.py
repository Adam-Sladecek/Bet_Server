...
def scrape_fn(event: threading.Event, import_queue: Queue):
    ...
    result_queue = Queue(maxsize=number_of_sbs)
    ...
    group_results_thread = threading.Thread(target= group_results, args = (command_queues, result_queue, event, number_of_sbs))    
